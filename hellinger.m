function [d_kl,D_KL] = hellinger(model_path,I,reps,pi_1,pi_2)
	new_halo_name = 'HLD';%saved as this
	obs_path = 'data/obsdata_s4t2m720.dat';%this can be anything
	pi_true = NaN;
	...
	if mi0.num_models<pi_2
		D_KL=zeros(1,reps);
		d_kl = mean(D_KL);

		sepr(['No data for ',num2str(pi_1),' v. ',num2str(pi_2),' models.  [',model_path,']'])
	else
		...
		for i=1:reps
			...
			for k=1:K
				c = (sqrt(mi.f(k,pi_1))-sqrt(mi.f(k,pi_2))).^2;
				if isfinite(c)
					d_kl = d_kl + c;
				end
			end
