function [d_kl,D_KL] = kl_divergence(model_path,I,reps,pi_1,pi_2)
	...
	for i=1:reps
		...
		for k=1:K
			c = mi.f(k,2+pi_1) * log(mi.f(k,2+pi_1)/mi.f(k,2+pi_2));
			if isfinite(c)
				d_kl = d_kl + c;
			end
		end
	...
	sepr(['D_kl = ',num2str(d_kl),' for I=',num2str(I),', repeated ',num2str(reps),' times for ',num2str(pi_1),' v. ',num2str(pi_2),' models.  [',model_path,']'])